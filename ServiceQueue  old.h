...
class ServiceQueue {
  ...
  private:
    ...
    vector<int> queue1;  // front
    vector<int> buzzer;  // vector of pointers
    vector<node*> nodes;
  ...
  public:
	...
	int  length() {
             ...
             return queue1.size();   // placeholder
	...
	int  give_buzzer() {
	    int b;
        if (buzzer.size() != 0){        // if the size is not 0, there's reusable buzzers
             b = buzzer.back();         // assign b as last of vector value
            buzzer.pop_back();          // remove last vector value
        }

        else
            b = queue1.size();          // assign b as the size of the vector
                                        // this way the reassigned buzzer is back in line
        queue1.push_back(b);            // send b to the back of the vector.
	...
	int seat() {
        ...
        if(queue1.size() ==  0)
            ...
            buzzers  = queue1[0];        // take  the first element to delete

            buzzer.push_back(buzzers);   // return  the free  buzzer to the list of usable
        }
           return buzzers;  // placeholder
